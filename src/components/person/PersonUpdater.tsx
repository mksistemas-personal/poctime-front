...
interface PersonUpdaterProps {
    visible: boolean;
    personToUpdate: IPerson | null;
    onHide: () => void;
    onSave?: (person: IPerson) => void;
}

const PersonUpdater: React.FC<PersonUpdaterProps> = ({ visible, personToUpdate: initialPerson, onHide, onSave }) => {
    ...
    useEffect(() => {
        if (initialPerson && visible) {
            reset(initialPerson as any);
        }
    }, [initialPerson, visible, reset]);
