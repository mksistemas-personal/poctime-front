...
export class OrganizationService {
    ...
    static async getOrganizationsFromList(ids?: string[]): Promise<ISlice<IOrganizationView>> {
        try {
            ...
            let url: string = '';
            if (idsParam === undefined || idsParam === null || idsParam === '')
                url = `${OrganizationService.API_URL}/projection/from-list`;
            else
                url = `${OrganizationService.API_URL}/projection/from-list?ids=${idsParam?.toString()}`;
